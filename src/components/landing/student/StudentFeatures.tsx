  ...
  return (
    <section className="py-24 md:py-32 px-6 bg-white border-y border-[var(--color-border)]">
      <div className="max-w-6xl mx-auto">
        ...
        {/* "Học theo cách của bạn" - Highlighted Section */}
        <motion.div
          initial={{ opacity: 0, y: 40 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          ...
          className="mt-6"
        >
          <div className="card-warm p-8 md:p-10 border-2 border-[var(--color-accent)]/30">
            <div className="flex items-start gap-4 mb-6">
              <div className="w-14 h-14 rounded-2xl bg-[var(--color-accent)] flex items-center justify-center text-white shrink-0">
                <Brain size={28} />
              </div>
              <div>
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[var(--color-accent)]/10 text-[var(--color-accent)] text-xs font-medium mb-3">
                  <Sparkles size={12} /> ĐIỂM NỔI BẬT
                </div>
                <h3 className="font-serif text-2xl md:text-3xl text-[var(--color-text)] mb-3">
                  "Học theo cách của bạn"
                </h3>
                <p className="text-lg text-[var(--color-text-muted)] leading-relaxed">
                  AI không ép bạn theo lộ trình chuẩn hóa. Thay vào đó, hệ thống{" "}
                  <span className="font-medium text-[var(--color-text)]">sinh nội dung học tập dựa trên cách hiểu riêng</span>{" "}
                  của bạn — giải thích theo phong cách phù hợp, tốc độ phù hợp, ví dụ phù hợp với sở thích của bạn.
                </p>
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <div className="p-4 rounded-lg bg-[var(--color-bg)]">
                <p className="text-sm font-medium text-[var(--color-text)] mb-1">Adaptive Learning</p>
                <p className="text-xs text-[var(--color-text-muted)]">Nội dung tự động điều chỉnh theo trình độ</p>
              </div>
              <div className="p-4 rounded-lg bg-[var(--color-bg)]">
                <p className="text-sm font-medium text-[var(--color-text)] mb-1">Personal Examples</p>
                <p className="text-xs text-[var(--color-text-muted)]">Ví dụ gần gũi với sở thích cá nhân</p>
              </div>
              <div className="p-4 rounded-lg bg-[var(--color-bg)]">
                <p className="text-sm font-medium text-[var(--color-text)] mb-1">Your Pace</p>
                <p className="text-xs text-[var(--color-text-muted)]">Học nhanh hay chậm, tùy theo bạn</p>
              </div>
            </div>
          </div>
        </motion.div>
